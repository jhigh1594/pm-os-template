#!/usr/bin/env python3
"""
Pull specific Granola meetings by ID and save to Company/meeting-notes/granola.
"""
...
def extract_meeting_id(url: str) -> str:
    ...
    # URL format: https://notes.granola.ai/t/{meeting_id}-{suffix}
    parts = url.split('/t/')
    if len(parts) > 1:
        meeting_id_part = parts[1].split('-00b881l8')[0]  # Remove suffix if present
...
def create_markdown_file(meeting_id: str, output_dir: Path, cache_reader: GranolaCacheReader) -> Path:
    ...
    # Get transcript, documents, and Granola summary
    ...
    # Also get the raw document to check for summary/overview fields
    ...
    # Add summary - try multiple sources
    summary_added = False
    
    # First try Granola's AI summary from documentPanels
    ...
    # Then try overview field from document
