...
### Phase 5.5: Experiment Design

When iteration requires a controlled test rather than just shipping a fix:

1. **Frame the hypothesis**: "We believe [change] will cause [behavior] among [segment], measured by [metric], resulting in [business outcome]"
2. **Define guardrail metrics**: What must NOT get worse when we test this? List 2-3 guardrails
3. **Choose rollout mechanism**: Feature flag / A/B test / staged rollout / fake door
4. **Set minimum detectable effect**: What % improvement justifies the investment?
5. **Define runtime**: How long before results are statistically meaningful?
6. **Set kill criteria**: At what point does a negative result halt the experiment?

**Experiment brief:**
```
Hypothesis: [If X, then Y, resulting in Z]
Primary metric: [Metric + target threshold]
Guardrails: [Metrics that must not degrade — e.g., error rate, p99 latency]
Duration: [N days at Y% traffic]
Kill criteria: [Stop if Z happens]
```

### Phase 6: Growth Signal Review

Monthly synthesis connecting post-launch learnings to growth dynamics.

**Growth model check** — For the feature you've shipped:
- **Acquisition**: Is this driving new user/account acquisition? (Referral, word-of-mouth, press?)
- **Activation**: Does it help new users reach their first "aha moment" faster?
- **Retention**: Does it give users a reason to return daily/weekly?
- **Revenue**: Does it enable expansion, reduce churn, or justify price increases?
- **Referral**: Does it make users more likely to recommend the product?

If the answer to 3+ is "no," the feature may be local optimization — valuable, but not a growth driver. Factor this into prioritization of the next iteration cycle.

**Add to** `📚 Knowledge/Growth/growth-signals-[YYYY-MM].md`:
- Feature name + current learning phase (week 1 / month 1 / long-term)
- Top metric movement vs. target
- Growth loop impact (if any identified above)
